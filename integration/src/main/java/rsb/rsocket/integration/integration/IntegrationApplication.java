...
public class IntegrationApplication {
	...
	IntegrationFlow greetingFlow(ClientRSocketConnector clientRSocketConnector) {
		var names = new String[] { "Mario", "Richard", "Michelle", "Natalie", "Madhura",
				"Violetta", "Yuxin", "Olga", "Rob", "Jane", "Arun", "Heinz", "Venkat" };
	...
	IntegrationFlow greetingsResponseFlow() {
		return IntegrationFlows.from(this.channel())
				...
				.handle(new GenericHandler<Flux<GreetingResponse>>() {
					@Override
					public Object handle(Flux<GreetingResponse> responses,
							MessageHeaders headers) {
						responses.subscribe(gr -> log.info(gr.toString()));// todo there
																			// has to be
																			// someway to
																			// flatten
																			// this and
